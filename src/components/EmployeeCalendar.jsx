    ...
    try {
      ...
      if (data.success) {
        ...
        if (userAttendance) {
          ...
          setStats(prev => ({ ...prev, present: userAttendance.presentDays, absent: userAttendance.absentDays, total: userAttendance.totalDays }));
        } else {
          setAttendance([]);
          setStats(prev => ({ ...prev, present: 0, absent: 0, total: 0 }));
        }
      }
    } catch (error) {
                ...
                <div key={index} className="aspect-square">
                  {day ? (
                    <div className={`w-full h-full border-2 rounded-lg p-1 ${getCellStyle(day)}`}>
                      ...
                      {leave && <div className="text-xs mt-1 truncate">🏖️ Leave</div>}
                      {!leave && att && (
